...
export class StreamResolver {
  constructor(private readonly indexer: IndexerClient, private readonly engine: TorrentEngine, private readonly perCandidateTimeoutMs = 5000, private readonly embedFallback: EmbedFallback = new PublicEmbedFallback(), private readonly subtitleClient = new WyzieSubtitleClient()) {}
  ...
  private async tryCandidate(candidate: RankedCandidate, query: ResolverQuery): Promise<Omit<ResolverResult, 'attempted'>> {
    ...
    const local = selectSubtitleFiles(manifest.files);
    const existing = new Set(local.map((item) => item.lang));
    ...
    if (!existing.has('ara') || !existing.has('eng')) { try { subtitles.push(...await this.subtitleClient.search(query, existing)); } catch { /* subtitle provider is optional */ } }
