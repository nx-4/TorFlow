...
export class StreamResolver {
  ...
  async findStream(query: ResolverQuery): Promise<ResolverResult> {
    if (!query.title?.trim()) throw new Error('title is required');
    ...
    for (const candidate of ranked) {
      try {
        const result = await withTimeout(this.tryCandidate(candidate), this.perCandidateTimeoutMs, 'candidate timeout');
  ...
  private async tryCandidate(candidate: RankedCandidate): Promise<Omit<ResolverResult, 'attempted'>> {
    ...
    const file = manifest.files.find((item) => item.mimeType.startsWith('video/') || item.mimeType.startsWith('audio/')) ?? manifest.files[0];
