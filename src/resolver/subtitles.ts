...
export class OpenSubtitlesClient implements SubtitleProvider {
  ...
  async search(query: ResolverQuery, existing: Set<string>, requestedLanguage?: string): Promise<SubtitleTrack[]> {
    ...
    const headers = { accept: 'application/json', 'Api-Key': this.apiKey, 'User-Agent': 'Streamix_Hub v1.2.0' }; const url = new URL(`${this.baseUrl}/subtitles`); url.searchParams.set('languages', requestedLanguage ?? 'ar,en,fr,es,de,it,tr,pt,ru,nl,pl,ko,ja'); url.searchParams.set('order_by', 'download_count'); url.searchParams.set('order_direction', 'desc');
