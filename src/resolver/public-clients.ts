...
function magnet(hash: string, name: string): string {
  const params = [`xt=urn:btih:${encodeURIComponent(hash)}`, `dn=${encodeURIComponent(name)}`];
...
export class YtsClient implements IndexerClient {
  ...
  async search(query: ResolverQuery, signal?: AbortSignal): Promise<TorrentCandidate[]> {
    if (query.season !== undefined || query.episode !== undefined) return [];
    const url = new URL(`${this.baseUrl}/list_movies.json`);
    url.searchParams.set('query_term', query.imdb_id ?? query.title);
    ...
    return (payload.data?.movies ?? []).flatMap((movie) => (movie.torrents ?? []).map((torrent) => {
      const title = `${movie.title ?? query.title} ${movie.year ?? query.year ?? ''} ${torrent.quality ?? ''} ${torrent.type ?? ''} x264`;
      return { magnet: torrent.hash ? magnet(torrent.hash, title) : '', title, seeders: Number(torrent.seeds ?? torrent.peers ?? 0), leechers: Number(torrent.peers ?? 0), size: Number(torrent.size_bytes ?? 0), source: 'yts' };
...
export class TorrentioClient implements IndexerClient {
  ...
  async search(query: ResolverQuery, signal?: AbortSignal): Promise<TorrentCandidate[]> {
    ...
    const id = query.imdb_id ?? (query.tmdb_id ? `tmdb:${query.tmdb_id}` : encodeURIComponent(query.title));
    ...
    const payload = await response.json() as { streams?: Array<{ name?: string; title?: string; url?: string; infoHash?: string; behaviorHints?: { bingeGroup?: string } }> };
    return (payload.streams ?? []).map((stream) => {
      const title = stream.name ?? stream.title ?? 'Torrentio result';
      const hash = stream.infoHash ?? stream.url?.match(/urn:btih:([^&/]+)/i)?.[1];
      const seedMatch = title.match(/(?:👤|seed(?:s|ers)?[: ]*)\s*(\d+)/i);
      return { magnet: hash ? magnet(hash, title) : (stream.url?.startsWith('magnet:?') ? stream.url : ''), title, seeders: Number(seedMatch?.[1] ?? 0), source: 'torrentio' };
    }).filter((candidate) => candidate.magnet);
  }
}
...
export class MultiIndexerClient implements IndexerClient {
  ...
  async search(query: ResolverQuery, signal?: AbortSignal): Promise<TorrentCandidate[]> {
    const results: TorrentCandidate[] = [];
    for (const client of this.clients) {
      try { results.push(...await client.search(query, signal)); } catch { /* a public source may be unavailable; continue */ }
    }
    return results;
  }
}

export function createPublicIndexer(timeoutMs = 3500): IndexerClient {
  return new MultiIndexerClient([new YtsClient(undefined, timeoutMs), new TorrentioClient(undefined, timeoutMs)]);
}
